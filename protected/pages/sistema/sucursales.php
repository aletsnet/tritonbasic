...
class sucursales extends TPage
{
	public $i = 0;
	
	public function onInit($param){
		$this->master->titulo->Text = "Sucursales y Accesos";
	...
	public function btnEditar_OnClick($sender, $param){
		...
		$row  = LCtSucursales::finder()->find("borrado = 0 AND id_sucursales = ?", $keys[$item->itemIndex]);
		if($row instanceof LCtSucursales){
			if($row->ticket_logo != ""){
				$this->foto->ImageUrl = $row->ticket_logo;
				$this->file->value = $row->ticket_logo;
			}
			$this->id_sucursales->value = $row->id_sucursales;
			$this->dgAccesos->DataSource = LCtAccesos::finder()->findAll(" activo = 1 AND id_sucursal = ? ",$row->id_sucursales);
			$this->dgAccesos->dataBind();
			
			$this->txtSucursal->Text = $row->sucursal;
	...
	protected function getDataRows($offset,$rows)
    {
		...
		if($this->txtBuscar->text != ""){
			$where .= " AND (sucursal LIKE :buscar
		...
		$ct_buscar->OrdersBy['id_sucursales'] = 'asc';
        $ct_buscar->Limit = $rows;
        $ct_buscar->Offset = $offset;
		
        $tabla = LCtSucursales::finder()->findAll($ct_buscar);
    ...
    protected function getRowCount()
    {
        ...
        $var = LCtSucursales::finder()->count($ct_buscar);
